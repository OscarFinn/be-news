...
const seed = ({ topicData, userData, articleData, commentData }) => {
  ...
  .then((articleTitleId) => {
    const articleLookup = createLookupObject(articleTitleId.rows, 'title', 'article_id')
...
function insertTopicsData(topics) {
  const sqlString = format(
    `INSERT INTO topics (description, slug, img_url) VALUES %L`, topics
  );
...
function insertUsersData(users) {
  const sqlString = format(
    `INSERT INTO users (username, name, avatar_url) VALUES %L`, users
  );
  return db.query(sqlString);
}
...
function insertArticleData(articles) {
  const sqlString = format(
    `INSERT INTO articles (title, topic, author, body, created_at, votes, article_img_url) VALUES %L RETURNING article_id, title`, articles
  );
  return db.query(sqlString);
}
...
function insertCommentData(comments) {
  const sqlString = format(
    `INSERT INTO comments (article_id, body, votes, author, created_at) VALUES %L `, comments
  );
  return db.query(sqlString);
}
